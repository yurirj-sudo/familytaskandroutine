...
Usuário pertence a uma família. O `familyId` fica no documento `/users/{uid}` e também
no custom claim do Firebase Auth token para uso nas Security Rules.
...
```typescript
{
  ...
  settings: {
    ...
    requireTaskApproval: boolean;       // se true: filho submete → pai aprova antes dos pontos
...
```typescript
{
  ...
  emoji?: string;                       // ícone visual da tarefa
  ...
  pointsOnComplete: number;             // pontos ganhos ao concluir
  pointsOnMiss: number;                 // pontos perdidos ao falhar (negativo, só mandatory)
  assignedTo: string[] | 'all';         // uids ou 'all'
...
**Regra de negócio**: O resgate é **automático** — ao solicitar, se `member.totalPoints >= prize.pointsCost`,
o sistema debita os pontos e cria o redemption com status `approved` em uma transação Firestore.
Não há fluxo de aprovação manual para prêmios.
...
```typescript
{
  ...
  year: number;
  pointsEarned: number;
  pointsLost: number;                  // valor absoluto
  pointsSpent: number;                 // gasto em resgates
  finalScore: number;                  // earned - lost - spent
...
- Configurar settings da família (`pointsMode`, `requireTaskApproval`, `requirePhotoProof`, etc.)
...
- Sem interação com tarefas ou pontos
...
- Cloud Function `processMissedTasks` roda diariamente às 00:05 BRT

### Tarefa opcional
- Concluída: +`pointsOnComplete` (bônus — pode compensar perdas em mandatory)
...
| Tarefa submetida (aguardando aprovação) | Admin | "📋 [Nome] concluiu [Tarefa] — aguarda aprovação" |
...
2. Confirme a fase atual e feature em desenvolvimento
3. Mantenha a estrutura de arquivos definida acima
4. Nunca altere o modelo de dados sem revisar Security Rules e Cloud Functions impactadas
